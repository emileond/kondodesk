import { logger, task, AbortTaskRunError } from '@trigger.dev/sdk/v3';
import { createClient } from '@supabase/supabase-js';
import dayjs from 'dayjs';
import { toUTC, calculateExpiresAt } from '../utils/dateUtils';
import ky, { HTTPError } from 'ky';

// Initialize Supabase client once
const supabase = createClient(
    process.env.SUPABASE_URL as string,
    process.env.SUPABASE_SERVICE_KEY as string,
);

const GRAPH_BASE = 'https://graph.microsoft.com/v1.0';
const DB_BATCH_SIZE = 50;

/**
 * Reusable helper to refresh the Microsoft OAuth token.
 */
const refreshToken = async ({ integration_id, refresh_token, scopes }) => {
    logger.log('Refreshing Microsoft token...', { integration_id });
    try {
        const res = await ky
            .post('https://login.microsoftonline.com/common/oauth2/v2.0/token', {
                body: new URLSearchParams({
                    client_id: process.env.MICROSOFT_CLIENT_ID as string,
                    client_secret: process.env.MICROSOFT_CLIENT_SECRET as string,
                    refresh_token: refresh_token,
                    grant_type: 'refresh_token',
                    scope: scopes.join(' '),
                }),
            })
            .json<any>();

        if (res.error) {
            throw new AbortTaskRunError(`Token refresh failed: ${res.error_description}`);
        }

        await supabase
            .from('user_integrations')
            .update({
                access_token: res.access_token,
                refresh_token: res.refresh_token,
                expires_at: calculateExpiresAt(res.expires_in),
                status: 'active',
            })
            .eq('id', integration_id);

        logger.log('Token refreshed successfully.');
        return res;
    } catch (error) {
        if (error instanceof HTTPError) {
            const responseBody = await error.response.text();
            logger.error('Microsoft token refresh API error:', { body: responseBody });
            throw new AbortTaskRunError(`Token refresh failed. Response: ${responseBody}`);
        }
        throw error;
    }
};

export const microsoftSync = task({
    id: 'microsoft-sync',
    maxDuration: 3600,
    onSuccess: async (payload) => {
        await supabase
            .from('user_integrations')
            .update({ last_sync: toUTC(), status: 'active' })
            .eq('id', payload.id);
        logger.log(`Successfully synced Microsoft integration for ID: ${payload.id}`);
    },
    onFailure: async (payload) => {
        await supabase.from('user_integrations').update({ status: 'error' }).eq('id', payload.id);
        logger.log(`Failed to sync, status changed to error for integration: ${payload.id}`);
    },
    run: async (payload: any) => {
        logger.log(`Starting Microsoft sync for integration ID: ${payload.id}`);
        let {
            access_token,
            refresh_token,
            scopes,
            id: integration_id,
            workspace_id,
            user_id,
        } = payload;

        if (!scopes || scopes.length === 0) {
            logger.warn('No scopes found for this integration. Skipping sync.', { integration_id });
            return;
        }

        // --- Encapsulated Sync Logic ---

        const syncTasks = async (currentToken: string) => {
            const headers = { Authorization: `Bearer ${currentToken}`, Accept: 'application/json' };
            const activeApiTaskIds: string[] = [];
            const taskListsResponse = await ky
                .get(`${GRAPH_BASE}/me/todo/lists`, { headers })
                .json<any>();

            for (const list of taskListsResponse.value || []) {
                let nextLink: string | null =
                    `${GRAPH_BASE}/me/todo/lists/${list.id}/tasks?$filter=status ne 'completed'`;
                while (nextLink) {
                    const pageResponse = await ky.get(nextLink, { headers }).json<any>();
                    const tasksOnPage = pageResponse.value || [];

                    if (tasksOnPage.length > 0) {
                        // FIX: Prepare all upsert payloads for the current page first.
                        const upsertPayloads = tasksOnPage.map((task) => {
                            activeApiTaskIds.push(task.id);
                            return {
                                name: task.title,
                                description: task.body?.content || null,
                                workspace_id,
                                integration_source: 'microsoft',
                                external_id: task.id,
                                host: 'https://to-do.office.com',
                                assignee: user_id,
                                creator: user_id,
                                status: 'pending',
                            };
                        });

                        // FIX: BATCHING LOGIC RESTORED
                        for (let i = 0; i < upsertPayloads.length; i += DB_BATCH_SIZE) {
                            const batch = upsertPayloads.slice(i, i + DB_BATCH_SIZE);
                            await supabase.from('tasks').upsert(batch, {
                                onConflict: 'integration_source, external_id, host, workspace_id',
                            });
                        }
                    }
                    nextLink = pageResponse['@odata.nextLink'] || null;
                }
            }
            // Reconcile tasks
            const query = supabase
                .from('tasks')
                .update({ status: 'completed', completed_at: toUTC() })
                .eq('workspace_id', workspace_id)
                .eq('integration_source', 'microsoft')
                .eq('status', 'pending');
            if (activeApiTaskIds.length > 0) {
                query.not('external_id', 'in', `(${activeApiTaskIds.join(',')})`);
            }
            await query;
        };

        const syncCalendars = async (currentToken: string) => {
            const headers = { Authorization: `Bearer ${currentToken}`, Accept: 'application/json' };
            const calendarsPage = await ky
                .get(`${GRAPH_BASE}/me/calendars`, { headers })
                .json<any>();
            const calendars = calendarsPage.value || [];

            for (let i = 0; i < calendars.length; i += DB_BATCH_SIZE) {
                const batch = calendars.slice(i, i + DB_BATCH_SIZE);
                await Promise.all(
                    batch.map((cal) =>
                        supabase.from('calendars').upsert(
                            {
                                integration_id,
                                external_id: cal.id,
                                name: cal.name,
                                color: cal.color || null,
                                source: 'microsoft',
                                is_enabled: true,
                                workspace_id,
                                user_id,
                            },
                            {
                                onConflict:
                                    'integration_id, external_id, source, workspace_id, user_id',
                            },
                        ),
                    ),
                );
            }

            const { data: dbCalendars } = await supabase
                .from('calendars')
                .select('id, external_id')
                .eq('integration_id', integration_id);
            const calendarIdMap = new Map((dbCalendars || []).map((c) => [c.external_id, c.id]));

            for (const cal of calendars) {
                let nextLink: string | null = `${GRAPH_BASE}/me/calendars/${cal.id}/events?$top=50`;
                while (nextLink) {
                    const page = await ky.get(nextLink, { headers }).json<any>();
                    const eventsOnPage = page.value || [];

                    if (eventsOnPage.length > 0) {
                        // FIX: Prepare all upsert payloads for the current event page.
                        const eventUpserts = eventsOnPage.map((ev) => ({
                            calendar_id: calendarIdMap.get(cal.id),
                            external_id: ev.id,
                            title: ev.subject,
                            description: ev.body?.content || null,
                            start_time: ev.start?.dateTime,
                            end_time: ev.end?.dateTime,
                            is_all_day: !!ev.isAllDay,
                            source: 'microsoft',
                            web_link: ev.webLink,
                            workspace_id,
                            user_id,
                        }));

                        // FIX: BATCHING LOGIC RESTORED
                        for (let i = 0; i < eventUpserts.length; i += DB_BATCH_SIZE) {
                            const batch = eventUpserts.slice(i, i + DB_BATCH_SIZE);
                            await supabase.from('events').upsert(batch, {
                                onConflict:
                                    'calendar_id, external_id, source, workspace_id, user_id',
                            });
                        }
                    }
                    nextLink = page['@odata.nextLink'] || null;
                }
            }
        };

        // --- Main Orchestration Logic ---

        const performSync = async (token: string) => {
            logger.log('Checking scopes to determine sync actions...', { scopes });

            if (scopes.includes('Tasks.ReadWrite')) {
                logger.log('Tasks scope found, syncing To Do tasks.');
                await syncTasks(token);
            }
            if (scopes.includes('Calendars.ReadWrite')) {
                logger.log('Calendars scope found, syncing Calendar events.');
                await syncCalendars(token);
            }
        };

        try {
            const tokenExpired =
                !payload.expires_at || dayjs().utc().isAfter(dayjs(payload.expires_at));
            if (tokenExpired && refresh_token) {
                const newTokens = await refreshToken({ integration_id, refresh_token, scopes });
                access_token = newTokens.access_token;
            }

            try {
                await performSync(access_token);
            } catch (error) {
                if (error instanceof HTTPError && error.response.status === 401) {
                    if (!refresh_token)
                        throw new AbortTaskRunError('No refresh_token available for 401 recovery.');
                    const newTokens = await refreshToken({ integration_id, refresh_token, scopes });
                    await performSync(newTokens.access_token);
                } else {
                    throw error;
                }
            }

            return { success: true };
        } catch (error) {
            logger.error(`Error syncing Microsoft integration ID ${payload.id}:`, error);
            throw error;
        }
    },
});
