...
const MondayIntegrationCard = () => {
    ...
    const { data: integration, isLoading, isPending } = useUserIntegration(user?.id, 'monday');
    const deleteIntegration = useDeleteIntegration(user.id, 'monday');
    const updateIntegrationConfig = useUpdateIntegrationConfig(user.id, 'monday');
    ...
    const handleConnect = () => {
        ...
        // Replace with actual Monday.com OAuth URL and client ID
        window.location.href =
            'https://auth.monday.com/oauth2/authorize?client_id=76dfc0cfee01cac6dce794c99828b813&redirect_uri=https://weekfuse.com/integrations/oauth/callback/monday&response_type=code';
    };
    ...
    const handleDisconnect = () => {
        ...
        deleteIntegration.mutate(
            ...
            },
            {
                onSuccess: () => {
                    setStatus('inactive');
                    toast.success('Monday Integration disconnected');
                ...
                onError: (error) => {
                    ...
                    toast.error('Failed to disconnect Monday Integration');
                },
                onSettled: () => {
                    setLoading(false);
                },
    ...
    const handleReset = () => {
        ...
        deleteIntegration.mutate(
            ...
            },
            {
                onSuccess: () => {
                    setStatus('inactive');
                    toast.success('Monday Integration reset');
                },
                onError: (error) => {
                    console.error('Error resetting Monday:', error);
                    toast.error('Failed to reset Monday Integration');
                },
                onSettled: () => {
                    setLoading(false);
                },
    ...
    const onSubmit = (data) => {
        ...
        updateIntegrationConfig.mutate(
            ...
            {
                onSuccess: () => {
                    toast.success('Monday configuration saved');
                    onClose();
                },
                onError: (error) => {
                    console.error('Error saving Monday configuration:', error);
                    toast.error('Failed to save Monday configuration');
                },
                onSettled: () => {
                    setLoading(false);
                },
    ...
    return (
        <>
            <IntegrationCard
                ...
                isLoading={loading}
                isPending={isPending}
                description="Import all Monday.com items from boards visible to you."
                status={status}
            ...
            <Modal isOpen={isOpen} onClose={onClose}>
                <ModalContent>
                    <form onSubmit={handleSubmit(onSubmit)} id="monday-config">
                        <ModalHeader>Monday Configuration</ModalHeader>
                        <ModalBody>
                            <div className="space-y-6">
                                <div className="space-y-3">
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            ...
                                            >
                                                <Radio value="auto">
                                                    Automatically update in Monday.com
                                                </Radio>
                                                <Radio value="prompt">
                                                    Ask before updating in Monday.com
                                                </Radio>
                                                <Radio value="never">
                                                    Do nothing in Monday.com
                                                </Radio>
                                            </RadioGroup>
                                        )}
                                    />
                                ...
                                <div className="space-y-3">
                                    ...
                                    <p className="text-default-600 text-sm">
                                        Choose a project for all tasks imported from Monday.com
                                    </p>
                                    <Controller
                                        name="project_id"
                                        control={control}
